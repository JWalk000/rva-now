  ...
  return (
    <div className="min-h-screen bg-[#F3F0EB] pb-24">
      <div className="bg-[#14121A] px-5 pb-8 pt-8 text-white">
        ...
        <h1 className="mt-2 text-3xl font-extrabold">List An Event</h1>
      ...
      <div className="px-4 py-5">
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4 rounded-2xl border border-[#E6E0D8] bg-white p-5">
          <label className="block">
            <span className="text-sm font-bold text-[#14121A]">Event title</span>
            <input required value={title} onChange={(e) => setTitle(e.target.value)} className="mt-2 w-full rounded-xl border border-[#E6E0D8] px-4 py-3 text-sm" />
          </label>
          <label className="block">
            <span className="text-sm font-bold text-[#14121A]">Neighborhood</span>
            <select value={neighborhood} onChange={(e) => setNeighborhood(e.target.value)} className="mt-2 w-full rounded-xl border border-[#E6E0D8] px-4 py-3 text-sm">
              {neighborhoods.map((n) => (
          ...
          <label className="block">
            ...
            <input required value={dateTime} onChange={(e) => setDateTime(e.target.value)} placeholder="Saturday 7:00 PM" className="mt-2 w-full rounded-xl border border-[#E6E0D8] px-4 py-3 text-sm" />
          </label>
          <label className="block">
            <span className="text-sm font-bold text-[#14121A]">Venue</span>
            <input required value={venue} onChange={(e) => setVenue(e.target.value)} className="mt-2 w-full rounded-xl border border-[#E6E0D8] px-4 py-3 text-sm" />
          </label>
          <label className="block">
            <span className="text-sm font-bold text-[#14121A]">Contact email</span>
            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-2 w-full rounded-xl border border-[#E6E0D8] px-4 py-3 text-sm" />
          </label>
          <label className="block">
            <span className="text-sm font-bold text-[#14121A]">Pitch</span>
            <textarea required value={pitch} onChange={(e) => setPitch(e.target.value)} rows={4} className="mt-2 w-full rounded-xl border border-[#E6E0D8] px-4 py-3 text-sm" />
          </label>
          ...
          <div>
            ...
            <div className="mt-3 space-y-2">
              {promotionTiers.map((option) => (
                ...
                >
                  <p className="font-bold text-[#14121A]">{option.label} · {option.price}</p>
        ...
        <p className="mt-4 text-center text-sm text-[#5A5560]">
          <Link href="/" className="underline">Back to Discover</Link>
        </p>
      </div>
    </div>
  );
...
export default function SubmitPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#F3F0EB] p-6">Loading…</div>}>
      <SubmitForm />
    </Suspense>
  );
}
