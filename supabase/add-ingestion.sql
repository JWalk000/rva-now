-- Paste into Supabase SQL Editor (run after setup-all.sql)
-- Adds URL fields + duplicate tracking for Posh/Eventbrite ingestion
...
-- Backfill sample URLs for UI testing
update public.events set
  source_platform = coalesce(source_platform, 'manual'),
  source_url = coalesce(source_url, 'https://rva-now.local/events/' || slug)
where source_url is null;

-- Posh scraped events use their own slug + external_id (see paste-ingestion-now.sql)

update public.events set
  source_platform = 'eventbrite',
  source_url = 'https://www.eventbrite.com/',
  ticket_url = 'https://www.eventbrite.com/',
  external_id = 'sample-eventbrite-001'
where slug = 'rva-001';
