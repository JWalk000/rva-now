  ...
  return (
    <div className="min-h-screen bg-[#0B0A10] pb-10 text-white">
      <div className="relative h-[46vh]">
        ...
        <Link href="/" className="absolute left-4 top-4 rounded-full bg-black/40 px-4 py-2 text-sm font-bold backdrop-blur">
          ← Back
        </Link>
        <div className="absolute bottom-0 left-0 right-0 p-5">
          ...
          <p className="text-sm font-semibold text-white/70">{event.day} · {event.time}</p>
          <h1 className="mt-2 text-3xl font-extrabold leading-tight">{event.title}</h1>
          <p className="mt-2 text-white/75">{event.venue} · {event.neighborhood} · {event.price}</p>
        </div>
      </div>
      ...
      <div className="space-y-4 px-5 py-6">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {event.vibe.map((v) => (
              <span key={v} className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold">{v}</span>
            ))}
          </div>
        </section>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 rounded-full border border-white/15 px-4 py-3 text-sm font-bold"
          >
            {saved ? '★ Saved' : '☆ Save'}
          </button>
          {ticketUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex-1 rounded-full bg-[#C44B2F] px-4 py-3 text-center text-sm font-bold text-white"
            >
              Get Tickets
            </a>
          ) : null}
